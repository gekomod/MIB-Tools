...
# Tabela wariantów w formacie CSV
...
def load_variant_table(variant):
    ...
    """Wczytuje tabelę wariantów z CSV."""
    variant_data = {}

    for row in reader:
        if row['Variant'] == variant:  # Pomijaj puste wiersze
            variant_data[row['Variant']] = row
            # Wyświetl informacje o wariancie
...
def get_variant_info(variant_table, variant):
    """Pobiera informacje o wariantach z tabeli."""
    return variant_table.get(variant, {})

def replace_number_in_file(config, old_number, new_number):
    """Zamienia wszystkie wystąpienia old_number na new_number w konfiguracji."""
...
def main():
    # Użyj domyślnej nazwy pliku lub podanej przez użytkownika
    ...
    # Zapytaj użytkownika o konwersję regionu
    ...
    # Zapytaj użytkownika o zamianę numeru
    ...
    config.optionxform = str  # Zachowaj wielkość liter w opcjach
            ...
            for option, value in config.items(section):
                if option == "RequiredVersionOfDM":
                   ...
                   config2.set(section, "Region4", '"CN"')
                     ...
                     # Pobierz informacje o wariantach
